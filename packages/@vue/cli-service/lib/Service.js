...
module.exports = class Service {
  ...
  resolvePkg (inlinePkg, context = this.context) {
    ...
    // 在 context 指定的目录下找 package.json，找到后同步的解析成 JSON 返回
  ...
  loadEnv (mode) {
    ...
    // 这样的文件，在里面配置需要添加到 process.env 上的环境变量
    load(localPath)
    load(basePath)
    ...
    if (mode) {
      ...
      const defaultNodeEnv = (mode === 'production' || mode === 'test')
        ? mode
        : 'development'
      // 如果没有配置 .env 文件，也没有显式指定 mode 为 'production'/'test'，
      // process.env.NODE_ENV 默认是 'development'
  ...
  resolvePlugins (inlinePlugins, useBuiltIn) {
    ...
    if (inlinePlugins) {
      // inlinePlugins 可能会覆盖 builtInPlugins
      ...
      // package.json 中引入的 projectPlugins 有可能会覆盖 builtInPlugins
  ...
  loadUserOptions () {
    ...
    if (fileConfig) {
      ...
      // 同时在 vue.config.js 和 package.json 的 vue 字段配置时，package.json 中的会被忽略
      resolved = fileConfig
